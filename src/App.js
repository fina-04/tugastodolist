...
const App = () => {
  const { inputRef, tasks, setTasks, alert, showAlert, isEditing, setIsEditing, editId, setEditId, name, setName, filter, setFilter, isColorsOpen, setIsColorsOpen } = useGlobalContext();
    ...
    <>
      <div className="container" onClick={hideColorsContainer}>
        ...
        <form className="head" onSubmit={addTask}>
          <input type="text" ref={inputRef} placeholder="New Task" value={name} onChange={(e) => setName(e.target.value)} />
        ...
        <DragDropContext onDragEnd={handleDragEnd}>{tasks.length > 0 ? <List /> : <p className="no-tasks">Your list is clear!</p>}</DragDropContext>
        {tasks.length > 2 && (
          <button className="btn-delete-all" onClick={deleteAll} title="Delete All Tasks (Completed and Uncompleted)!">
            Clear All
          </button>
        )}
        <DarkModeToggle />
